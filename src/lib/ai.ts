...
- 重い荷物の場合、以下の優先順位で荷物預かり（luggageアイテム）を含めること:
  1. 宿泊ありの日は**ホテル・宿に預ける**のを最優先（チェックイン前の午前中は「フロントに荷物を預ける」、チェックアウト後の午後は「フロントに荷物を預かってもらう」）
  2. 宿泊なし・日帰りの場合は**観光の起点となる主要駅のコインロッカー**を使う
  3. 空港の荷物預かりは、空港を実際に経由する移動がある場合のみ使う（空港を使わない日程では提案しない）
